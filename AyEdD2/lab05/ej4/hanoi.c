...
hanoi_t hanoi_init(unsigned int disk_count) {
    ...
    assert(hanoi != NULL);
    //inicializamos cada uno de los campos de hanoi
...
//Modifico esta funcion para que libere la memoria no solo de hanoi sino que tambien de cada uno de los campos de hanoi, nos asegura que libera correctamente la memoria

hanoi_t hanoi_destroy(hanoi_t hanoi) {
    assert(hanoi != NULL);
    hanoi->aux = stack_destroy(hanoi->aux); //lo que hago es liberar el espacio de hanoi->aux
    hanoi->source = stack_destroy(hanoi->source); //lo que hago es liberar el espacio de hanoi->source
    hanoi->target = stack_destroy(hanoi->target); //lo que hago es liberar el espacio de hanoi->target
    free(hanoi);
    return NULL;
}
